...
private[spark] class DAGScheduler(
  ...
  private def getMissingParentStages(stage: Stage): List[Stage] = {
    ...
    // 从 第一个 RDD 开始遍历其依赖， 宽依赖 生成 shuffle map stage
    // 窄依赖 继续 向前 追溯  ，直到 遇到第一个 宽依赖。
    def visit(rdd: RDD[_]): Unit = {
      if (!visited(rdd)) {
        visited += rdd
        val rddHasUncachedPartitions = getCacheLocs(rdd).contains(Nil)
        if (rddHasUncachedPartitions) {
          // rdd.dependencies  这个是 RDD的 一个属性
          for (dep <- rdd.dependencies) {
            dep match {
              case shufDep: ShuffleDependency[_, _, _] =>
                // 构建 或者创建 shuffle map stage , 由此可见 构建 shuffle map task 的时候 就是根据子RDD 对 父 RDD的依赖关系来 构建 父stage的
                val mapStage = getOrCreateShuffleMapStage(shufDep, stage.firstJobId)
                if (!mapStage.isAvailable) {
                  // 假如 map stage 没有 准备好 就加入 missing
                  missing += mapStage
                }
  ...
  //从这开始 构建 stage，只提交了一个final stage
  ...
  // 紧接着上面的 hanndle job submitted stage
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        ...
        if (missing.isEmpty) {
          ...
          //假如 没有 未提交的 父 parentstage   直接提交该 stage
          submitMissingTasks(stage, jobId.get)
        } else {
          // 假如有  未提交的 Parent stage 继续递归调用 遍历
          for (parent <- missing) {
            submitStage(parent)
          }
          waitingStages += stage
